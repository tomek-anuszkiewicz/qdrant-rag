...
from rag_qdrant.discovery import discover_markdown_files, is_included_path
...
class CliContractTests(unittest.TestCase):
    def test_removed_filter_options_are_reported_as_unknown(self):
        ...
        _, unknown = parser.parse_known_args(
            [".", "--source", "amiga", "--exclude", "private", "--no-root-notes"]
        )

        self.assertEqual(unknown, ["--exclude", "private", "--no-root-notes"])

    def test_include_dirs_is_required_for_indexing(self):
        args = create_parser().parse_args(
            [".", "--source", "amiga", "--include-dirs", "01*", "02*"]
        )

        self.assertEqual(args.path, ".")
        self.assertEqual(args.source, "amiga")
        self.assertEqual(args.include_dirs, ["01*", "02*"])
        self.assertIsNone(validate_indexing_arguments(args))

    def test_indexing_without_include_dirs_is_rejected(self):
        args = create_parser().parse_args([".", "--source", "amiga"])

        self.assertEqual(
            validate_indexing_arguments(args),
            "The --include-dirs option is required and must name one or more top-level directories.",
        )
...
class DiscoveryContractTests(unittest.TestCase):
    def test_ragignore_does_not_affect_discovery(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "included").mkdir()
            indexed_note = root / "included" / "note.md"
            indexed_note.write_text("# Indexed\n", encoding="utf-8")

            files = discover_markdown_files(root, ["included"])

        self.assertEqual(files, [indexed_note])

    def test_discovery_only_returns_selected_directory_files(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            (root / "02 Other").mkdir()
            (root / "02 Other" / "other.md").write_text("# Other\n", encoding="utf-8")
            (root / "_Private").mkdir()
            (root / "_Private" / "secret.md").write_text("# Secret\n", encoding="utf-8")

            files = discover_markdown_files(root, ["01*"])

        self.assertEqual(files, [selected_note])
        self.assertFalse(is_included_path(root_note, root, ["01*"]))
        self.assertTrue(is_included_path(selected_note, root, ["01*"]))
...
class LauncherContractTests(unittest.TestCase):
    ...
    def test_cli_help_uses_the_rag_qdrant_command_name(self):
        ...
        self.assertIn("rag_qdrant PATH --source NAME --include-dirs DIRS [OPTIONS]", cli_source)
        self.assertNotIn("amiga_rag [PATH] --source NAME [OPTIONS]", cli_source)


class CacheContractTests(unittest.TestCase):
    def test_collection_change_clears_file_hashes_and_preserves_image_descriptions(self):
        ...
        self.assertEqual(
            ...
            {"C:/docs/diagram.png": "Timing diagram"},
        )
...
class CommandContractTests(unittest.TestCase):
    ...
    def test_search_command_accepts_machine_readable_source_filter(self):
        args = create_search_parser().parse_args(
            ["Copper timing", "--source", "amiga,obsidian", "--limit", "3", "--json"]
        )
